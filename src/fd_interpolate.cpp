  ...
  Eigen::SparseMatrix<double> & W)
{
	...
	for (int i = 0; i < numPts; ++i)
	{
		...
		wVal.push_back({ i, Gx + (nx - 1)*(Gy + ny*Gz), (1 - xd)*(1 - yd)*(1 - zd) });
		wVal.push_back({ i, Gx + (nx - 1)*(Gy + ny*(Gz+1)), (1 - xd)*(1 - yd)*(zd) });
		wVal.push_back({ i, Gx + (nx - 1)*(Gy+1 + ny*Gz), (1 - xd)*(yd)*(1 - zd) });
		wVal.push_back({ i, Gx + (nx - 1)*(Gy+1 + ny*(Gz+1)), (1 - xd)*(yd)*(zd) });
		wVal.push_back({ i, Gx+1 + (nx - 1)*(Gy + ny*Gz), (xd)*(1 - yd)*(1 - zd) });
		wVal.push_back({ i, Gx+1 + (nx - 1)*(Gy + ny*(Gz+1)), (xd)*(1 - yd)*(zd) });
		wVal.push_back({ i, Gx+1 + (nx - 1)*(Gy+1 + ny*Gz), (xd)*(yd)*(1 - zd) });
		wVal.push_back({ i, Gx+1 + (nx - 1)*(Gy+1 + ny*(Gz+1)), (xd)*(yd)*(zd) });
	}
